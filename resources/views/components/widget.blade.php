...
<div
    {{ $attributes->merge(['class' => 'group border-gray-400 border-l-4 bg-white p-2 md:p-4 rounded-xl h-full hover:shadow-lg transition-shadow duration-300']) }}>
    <a href="{{ $attributes['href'] ?? '#' }}" class="flex flex-col h-full">
        ...
        <div class="flex flex-col h-full">
            <header class="flex justify-between  items-center">
                ...
                <x-icon icon="arrow-up-right" class="w-5 h-5 text-gray-400 group-hover:text-gray-600 transition-all duration-300 group-hover:mr-1" />
            </header>
