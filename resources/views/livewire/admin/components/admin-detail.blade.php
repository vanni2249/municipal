    ...
    <x-card>
        {{-- @if ($form->employee->admin) --}}
            <x-card-header class="flex justify-between items-center">
                <x-h2 value="Información del administrador" />
            </x-card-header>

            <x-card-body-grids>
                <x-card-body-grid label="Number" value="{{ $admin->number }}" class="col-span-6" />
                <x-card-body-grid label="Username" value="{{ $admin->username }}" class="col-span-6" />
           </x-card-body-grids>
        {{--  @else
