            ...
            <x-card class="rounded-xl">
                ...
                <header class="flex justify-between items-center">
                    <div class="flex flex-col space-x-4">
                        <h3 class="font-bold text-lg text-gray-900 line-clamp-1">{{ $user->name }} {{ $user->lastname }}</h3>
                    </div>
                    <div class="flex space-x-2">
                        <x-icon-button @click="$dispatch('open-modal', 'more-detail')" icon="eye" />
                        <x-button variant="light"  @click="$dispatch('open-modal', 'more-detail')" size="sm" label="Ver más" />
                ...
                <x-modal name="more-detail" title="Más detalles de {{ $user->name . ' ' . $user->lastname }}">
