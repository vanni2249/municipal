                ...
                <x-table>
                    <x-slot name="head">
                        <tr>
                            ...
                            <th class="p-2 w-1/4">Numero</th>
                            <th class="p-2 w-1/4">Agencia</th>
                            <th class="p-2 w-1/4">Status</th>
                            <th class="p-2 w-14">Accion</th>
                        </tr>
                    </x-slot>
                    <x-slot name="body">
                        @foreach (App\Data\Equipment::items() as $item)
                            
                        <tr class="border-t border-gray-200">
                            <td class="px-2 py-1">{{ $item['category'] }}</td>
                            <td class="px-2 py-1">{{ $item['numero'] }}</td>
                            <td class="px-2 py-1">{{ $item['agencia'] }}</td>
                            <td class="px-2 py-1">
        ...
        <div class="col-span-full lg:col-span-3">
            <x-card>
                <div class="grid grid-cols-2 gap-4">
                    @php
                        $items = [
                            ['title' => 'Totales', 'value' => '34', 'span' => 'full'],
                            ['title' => 'Departamento de Recreación y Deportes', 'value' => '23', 'span' => 1],
                            ['title' => 'Departemaneto de obras publicas', 'value' => '11', 'span' => 1],
                        ];
                    @endphp
                    @foreach ($items as $item)
                    <div class="bg-gray-100 col-span-{{ $item['span'] }} rounded p-2">
                        <small class="text-gray-600 text-xs line-clamp-1">{{ $item['title'] }}</small>
                    ...
                    <div class="bg-gray-100 col-span-2 rounded-md">
                        <header class=" p-2">
                            <h3 class="font-bold text-gray-800">Equipos mas usadas</h3>
                        </header>
                        @php
                            $interactions = [
                                ['title' => 'EQ-001', 'count' => 10],
                                ['title' => 'EQ-002', 'count' => 5],
                            ];
                        @endphp
